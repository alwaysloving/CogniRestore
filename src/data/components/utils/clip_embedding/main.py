...
import requests
#from src.data.components.utils.clip_embedding.utils.factory import create_model_and_transforms

import os
import src.data.components.utils.clip_embedding.utils as utils
import json
...
class FeatureGet:
    def __init__(self):
        self.device = "cuda:0"
        self.vlmodel, self.preprocess_train, _ = self._get_model()

    # def _get_model(self):
    #     vlmodel, preprocess_train, _ = utils.create_model_and_transforms(
    #         'ViT-H-14',
    #         pretrained='/root/autodl-fs/CognitionCapturer/model_pretrained/1c2b8495b28150b8a4922ee1c8edee224c284c0c/open_clip_pytorch_mode.bin',
    #         output_dict=True,
    #         precision='fp32',
    #         device=self.device)
    #     # todo 得到局部特征？
    #     vit = deepcopy(vlmodel.visual)
    #     vit.output_tokens=True
    #     return vlmodel, preprocess_train, vit
    def _get_model(self):
        # 使用第二个代码的方式，直接使用 open_clip.create_model_and_transforms
    ...
    def get_text_feature(self, type):
        ...
        with torch.no_grad():
            # 'train.npy' , 'test.npy'
            for text_file_name, img_file_name in zip(['texts_BLIP2_test.npy'], ['test_image.npy']):
                text_features_dict = {}
                ...
                torch.save(text_features_dict,
                           f"/root/autodl-fs/CognitionCapturer/model_pretrained/data_features/text_{type}_features_clip_dict_test.pt")
    def get_feature(self, type):
        ...
        with torch.no_grad():
            for i, file_name in enumerate(['test_image.npy']):
                image_features_dict = {}
                ...
                for j, img_path in enumerate(path):
                    ...
                    if type == 'aug':
                        img_path = img_path.replace("testing_images", "test_aug_images")
                ...
                # 保存路径
                save_path = f"/root/autodl-fs/CognitionCapturer/model_pretrained/data_features/image_{type}_features_clip_dict_test.pt"
...
if __name__ == "__main__":
    try:
        ...
        open_clip.add_model_config("/root/autodl-fs/CognitionCapturer/model_pretrained/clip/ViT-H-14.json")
        instance = FeatureGet()
